    ...
    can.extend(can.Map.prototype, {
        errors: function (attrs, newVal) {
            ...
            if(typeof(newVal) == "object"){
                can.each(newVal.options ,function(element,index){
                    _attr = attrs.replace("[]","."+element)
                });
                newVal =  "newVal" in newVal ? newVal.newVal : undefined;
            ...
            var errors = {}, self = this, addErrors = function (attr, funcs) {
                    can.each(funcs, function (func) {
                        var res = func.call(self, isTest ? newVal : self.attr(_attr));
                        if (res) {
                ...
                }, validations = this.constructor.validations || {}, isTest = attrs && attrs.length === 1&& arguments.length === 2&&newVal!=undefined;
            can.each(attrs || validations, function (funcs, attr) {
              
                if (typeof attr === 'number') {
                    attr = funcs;
                  
                    funcs = validations[attrs];
                }
